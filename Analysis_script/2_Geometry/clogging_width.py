...
if __name__ == "__main__":
    ...
    logging.info("call makeini.py with -f %s/master_width_ini.xml" % DIR)
    subprocess.call(["python", "makeini.py", "-f", "%s/master_width_ini.xml" % DIR])
    ...
    for TrajFile in TrajFiles:
        ...
        if float(Eframe) / float(frameRate)<2000:
            Trajdata.append([float(seed), float(width),float(case),float(Eframe) / float(frameRate)])
