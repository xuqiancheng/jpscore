...
void Line::pointIntersectionWithCircle(std::vector<Point> & interP, const Point& centre, double radius)
{
    ...
    if (p1._x == p2._x)
    {
        double delta = radius * radius - p1._x*p1._x;
        if (delta < 0)
        {
            return;
        }
        else if (delta = 0)
        {
            ...
            if (isBetween(pTemp))
            {
                interP.push_back(pTemp);
            }
            return;
        }
        else
        {
            Point pTemp1 = Point(p1._x, sqrt(delta)) + centre;
            Point pTemp2 = Point(p1._x, -sqrt(delta)) + centre;
            if (isBetween(pTemp1))
            {
    ...
    {
        double k1 = (p2._y - p1._y) / (p2._x - p1._x);
        double k2 = p2._y - k1 * p2._x;
        double a = k1 * k1 + 1;
        double b = 2 * k1*k2;
