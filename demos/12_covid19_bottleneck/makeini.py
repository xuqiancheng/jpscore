...
import sys
import itertools
from shutil import copy2, rmtree, move, copy
import numpy as np
...
def update_attrib_value(root, attr_tag, value):
    ...
    for r in root.iter(cor_tag):
        if attr in r.attrib:
            number = eval(r.attrib[attr])
            if isinstance(number, list) or isinstance(number, np.ndarray):
                r.attrib[attr] = str(value)
            else:
                r.attrib[attr]=str(int(number)-value)
# =======================================================
def make_file(masterfile, tree, result):
    ...
    for item in result:
        ...
        if not os.path.isfile(newfile):
            loggingerror("make_file: could not create file %s"%newfile)
            sys.exit(FAILURE)
